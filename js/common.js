...
$(document).on("pageshow", "div[data-role=page]", function () {
	...
	if (activePage == "pet-profile-listing" || activePage == "pet-profile" || activePage == "pet-profile-detail") {
	  ...
	  $(".my-home-link").addClass("active");
	 }

});
$(document).on("deviceready", function () {
    processScreenArrival("App load");
...
function login() {
	...
	{
		if ($.trim(val.value) == "") {
			$("input[name=" + val.name + "]").parent().next(".error-message").text("* Please enter a valid email address.");
			error++;
		} else if (val.name == "username" && validateEmail($.trim(val.value)) == false) {
			$("input[name=" + val.name + "]").parent().next(".error-message").text("* Please enter your password.");
			error++;
		}
...
function showAboutDialog() {
    ...
    window.fabric.Crashlytics.addLog("Dismissing App info screen.");
}


// view my profile page --------------------------
$(document).on("pagebeforeshow", "#profile", function () {
	processScreenArrival('My Profile');
...
$(document).on("pagebeforeshow", "#veterinarian-info", function () {
    processScreenArrival('Veterinarian Information');
...
$(document).on("pagebeforeshow", "#pet-profile-listing", function () {
    processScreenArrival('Pet Profiles');
...
function petProfile(petId, action) {
	...
	$.ajax({
		...
		dataType: "json",
		success: function (res) {
			window.fabric.Crashlytics.addLog("Displaying pet information and ending ajax request to get individual pet profile information for petid: "+petId+" at " + get_date_time());
    		window.fabric.Crashlytics.sendNonFatalCrash("");
			...
			if (response.response == "Y") {
				if (typeof response.profiles != "undefined" && response.profiles.length > 0) {
                    ...
                    $.each(response.profiles, function (k, val) {
						...
						html += '<div class="owl-item" style="'+pad+'"><div class="item"><a onclick="viewPetProfile(' + val.UPID + ')" href="javascript:void(0);"><img src="' + val.UIMAGE + '" alt="Murphy" title="Murphy" class="circle-img"/>' +
						'<h2>' + val.UPETNAME + '</h2>' +
						'</a> </div></div>';
                        if(coun == 3){
...
$(document).on("pagebeforeshow", "#travel-plan", function () {
    processScreenArrival('Travel Plan');
...
$(document).on("pagebeforeshow", "#change-password", function () {
    processScreenArrival('Change Password');
});


// view pet profile screen -----------------------------
$(document).on("pagebeforeshow", "#pet-profile-detail", function () {
    processScreenArrival('View Pet Profile');
});

// add/edit pet profile screen -----------------------------
$(document).on("pagebeforeshow", "#pet-profile", function () {
    processScreenArrival('Add-Edit Pet Profile');
});


// add-edit pet document screen -----------------------------
$(document).on("pagebeforeshow", "#add-edit-pet-document", function () {
    processScreenArrival('Add-Edit Pet Document');
});


// forgot password screen -----------------------------
$(document).on("pagebeforeshow", "#forgot-password", function () {
    processScreenArrival('Forgot Password');
});
...
$(document).on("pagebeforeshow", "#pet-document-listing", function () {
    processScreenArrival('Documents');
...
// function for updating user data after notification -----------------------------------------------
...
function processScreenArrival(screenName) {
    $(".error-message").html("");
    window.FirebasePlugin.setScreenName(screenName);
    window.fabric.Crashlytics.addLog("Arrived at page: " + screenName);
    window.fabric.Crashlytics.sendNonFatalCrash("");
}
